...
- `cdlc_residue_algebra`: proves the same bridge/adaptor identities over the
  ...
  `pragma Assume` statements. This is the accepted finite modular proof target.
- `cdlc_algebra`: a diagnostic model using Ada's built-in `type mod 97`. GNATprove
  currently leaves two bit-vector modular obligations unproved in that target,
  so it is not used as proof evidence.
...
Both accepted targets end with `0 errors, 0 warnings and 0 pragma Assume
