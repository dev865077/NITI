...
package body Lightning_Cdlc_Algebra is
   ...
   procedure Prove_HTLC_And_Adaptor_Synchronize
     (Oracle_Nonce  : Scalar;
      ...
      Challenge     : Scalar) is
   begin
      Prove_Oracle_HTLC_Compatibility
        (Oracle_Nonce, Oracle_Secret, Challenge);
      Prove_Oracle_PTLC_Compatibility
        (Oracle_Nonce, Oracle_Secret, Challenge);
   end Prove_HTLC_And_Adaptor_Synchronize;
   ...
   procedure Prove_HTLC_Route_Atomicity (Secret : Scalar) is
   begin
      Prove_HTLC_Correct_Secret_Settles (Secret);
   end Prove_HTLC_Route_Atomicity;
   ...
   end Prove_HTLC_Route_Wrong_Secret_Fails;
   ...
   procedure Prove_PTLC_Two_Hop_Route_Atomicity
     (Final_Secret : Scalar;
      First_Tweak  : Scalar;
      Second_Tweak : Scalar) is
      Routed_Secret : constant Scalar :=
        Route_Secret_2 (Final_Secret, First_Tweak, Second_Tweak);
   begin
      Prove_Point_Is_Scalar_Embedding (Final_Secret);
      Prove_Point_Is_Scalar_Embedding (First_Tweak);
      Prove_Point_Is_Scalar_Embedding (Second_Tweak);
      Prove_Point_Is_Scalar_Embedding (Routed_Secret);
   end Prove_PTLC_Two_Hop_Route_Atomicity;
   ...
   procedure Prove_PTLC_Two_Hop_Wrong_Secret_Fails
     (Final_Secret : Scalar;
      ...
      Correct_Secret : constant Scalar :=
        Route_Secret_2 (Final_Secret, First_Tweak, Second_Tweak);
   begin
      Prove_PTLC_Two_Hop_Route_Atomicity
        (Final_Secret, First_Tweak, Second_Tweak);
      Prove_PTLC_Wrong_Secret_Does_Not_Settle
        (Correct_Secret, Wrong_Secret);
   end Prove_PTLC_Two_Hop_Wrong_Secret_Fails;
   ...
   begin
      Prove_HTLC_Correct_Secret_Settles (Secret);
   end Prove_HTLC_Timeout_Does_Not_Refund_After_Redemption;

   procedure Prove_PTLC_Timeout_Does_Not_Refund_After_Redemption
     (Secret : Scalar) is
   begin
      Prove_PTLC_Correct_Secret_Settles (Secret);
   end Prove_PTLC_Timeout_Does_Not_Refund_After_Redemption;
