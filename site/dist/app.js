...
const modes = {
    htlc: {
        ...
        copy: 'The oracle announcement commits to h_x = SHA256(s_x). A receiver creates a hold invoice using h_x. After attestation, s_x settles the invoice.',
        left: 'h_x',
        right: 'SHA256(s_x)',
    ...
    ptlc: {
        ...
        copy: 'The ordinary DLC point S_x is the payment point. Each hop can receive a tweaked point and derive its upstream scalar after downstream settlement.',
        left: 'T_i = S_x + d_iG',
        right: 't_i = s_x + d_i',
...
function firstButton(buttons, groupName) {
...
function initProtocolStages() {
    const buttons = selectAll('[data-stage]');
    const title = select('[data-stage-title]');
    const equation = select('[data-equation]');
    const nodes = selectAll('[data-node]');
    const setStage = (stage, selected) => {
...
function initLightningModes() {
    const buttons = selectAll('[data-mode]');
    const title = select('[data-mode-title]');
    const copy = select('[data-mode-copy]');
    const left = select('[data-lock-left]');
    const right = select('[data-lock-right]');
    const note = select('[data-route-note]');
    const setMode = (mode, selected) => {
...
function initProofTabs() {
    const buttons = selectAll('[data-proof]');
    const name = select('[data-proof-name]');
    const body = select('[data-proof-body]');
    const checks = select('[data-proof-checks]');
    const setProof = (proof, selected) => {
    ...
    buttons.forEach((button) => {
        button.addEventListener('click', () => setProof(button.dataset.proof, button));
    });
    setProof('integer', firstButton(buttons, 'proof targets'));
}
function initCommandTabs() {
    const buttons = selectAll('[data-command]');
    const body = select('[data-command-body]');
    const setCommand = (command, selected) => {
        setPressed(buttons, selected);
        body.textContent = commands[command];
    };
    buttons.forEach((button) => {
        button.addEventListener('click', () => setCommand(button.dataset.command, button));
    });
    setCommand('bitcoin', firstButton(buttons, 'command examples'));
}
function initSignalCanvas() {
    const canvas = select('#signal-canvas');
    const context = canvas.getContext('2d');
    if (!context) {
        return;
    }
    const label = select('[data-signal-label]');
    const cycle = select('[data-reroute]');
    let state = 0;
    let frame = 0;
