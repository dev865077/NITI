...
\begin{document}
\begin{center}
{\LARGE\bfseries Cascading Discreet Log Contracts}\\[1.0em]
...
outcome also completes the bridge signatures and funds the child DLC. It enables contracts such as rolling exposures and
renewable fixed-term positions without new consensus rules.
\end{abstract}
...
All three targets completed with no unproved checks and no \code{pragma Assume}
...
serialisation, and the full protocol remain outside the machine-checked boundary.
...
This is sufficient to build a graph of conditional Bitcoin transactions in which
one DLC outcome activates another.\newpage
