...
| Layer 3: bilateral protocol | Two independent participants can negotiate, exchange adaptors, validate counterparty messages, and retain enough state for the prepared child path. | Role separation, message schema, funding validation, bilateral adaptor exchange, state retention, two-process execution, recovery, audit transcript, and malformed-message tests. | Two-process or isolated-role transcript, state-retention manifest, malformed-message rejection suite. | Gate defined; not satisfied by the single-party smoke harness alone. | v0.1 may still rely on a prototype bilateral protocol rather than production wallet UX or network transport. |
