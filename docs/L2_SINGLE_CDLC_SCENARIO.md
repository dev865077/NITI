...
fixture prevout and does not claim public mempool relay or confirmation.
...
The deterministic fixture starts from this prevout:

| Field | Value |
| --- | --- |
| Funding txid | `bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb` |
| Funding vout | `0` |
| Funding value | `100000 sat` |
| Spend key | Parent funding signer |

For live regtest/testnet execution, this prevout is replaced by a real funded
Taproot output to the parent funding signer. The value must remain
`100000 sat` unless the accounting table below is updated. Downstream txids in
this document are deterministic for the fixture prevout and will change when a
live funded outpoint is substituted.
...
| Field | Value |
| --- | --- |
| Input | parent funding txid `bbbb...bbbb:0` |
| Output 0 script | bridge signer P2TR script pubkey |
| Output 0 value | `99000 sat` |
| Fee | `1000 sat` |
| Adaptor point | activating attestation point `S_x` |
| Fixture txid without witness | `bc04c390d5168aabec083bdc82f8e57dc602175d6c750359c27af72ab33a27bd` |
...
| Field | Value |
| --- | --- |
| Input | parent CET txid `bc04c390d5168aabec083bdc82f8e57dc602175d6c750359c27af72ab33a27bd:0` |
| Output 0 script | child funding signer P2TR script pubkey |
| Output 0 value | `98500 sat` |
| Fee | `500 sat` |
| Adaptor point | activating attestation point `S_x` |
| Fixture txid without witness | `6329ccf07eba7d95f1a052a4d586d6ea10eebf5fb0b82c0b74c6db85d0f938ba` |
...
| Field | Value |
| --- | --- |
| Funding txid | `6329ccf07eba7d95f1a052a4d586d6ea10eebf5fb0b82c0b74c6db85d0f938ba` |
| Funding vout | `0` |
...
2. The parent CET adaptor signature verifies before completion using `S_x`.
3. Completing the parent CET with `s_x` produces a valid Taproot signature.
4. The scalar extracted from the completed parent CET signature equals `s_x`.
5. The completed parent CET contains output `0` to the bridge signer for
   `99000 sat`.
6. The bridge adaptor signature verifies before completion using the same
   `S_x`.
7. The bridge input spends the completed parent CET txid at vout `0`.
8. Completing the bridge with `s_x` produces a valid Taproot signature.
9. The scalar extracted from the completed bridge signature equals `s_x`.
10. The completed bridge contains output `0` to the child funding script for
    `98500 sat`.
...
3. The parent CET output value is not `99000 sat`.
4. The bridge output value is not `98500 sat`.
5. The bridge spends any outpoint other than the parent CET output `0`.
6. The child funding script is not the child funding signer P2TR script.
7. The oracle scalar extracted from either completed adaptor signature does not
