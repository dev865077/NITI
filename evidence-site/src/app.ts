...
const copy: Record<Language, Record<string, string>> = {
  pt: {
    ...
    ahaCopy:
      'Sem o segredo, a bridge é um rascunho criptográfico. Com o segredo, ela vira uma transação Bitcoin normal. Por isso o cDLC consegue encadear contratos sem deixar o próximo passo depender de um signer online na hora do evento.',
    ...
    footer:
      'Demo educativa em signet/testnet. Nada aqui é oferta financeira, garantia de dólar, stablecoin, custódia real ou fluxo mainnet.',
  },
  en: {
    play: 'Play',
    pause: 'Pause',
    ...
    heroLine:
      'Read the screenshot like a Bitcoiner: one input, one output, one fee, confirmations. The question is simple: why did this transaction only become valid after the oracle spoke?',
    ...
    pointCopy:
      'Parent funding. Parent CET. Bridge. Child funding. The whole story becomes a UTXO sequence. The contract is not an off-chain promise; it is a choreography of transactions prepared before the event and activated by the correct outcome.',
    mapEyebrow: 'Mental model',
    mapTitle: 'Think of three doors in a row.',
    mapCopy:
      'The first door holds the parent. The second is the bridge. The third is the child. Before the event, the locks are already installed. The oracle does not move sats; it reveals the password that makes the right signature close.',
    fundingEyebrow: 'Scene 1',
    fundingTitle: 'First, collateral enters the parent.',
    fundingCopy:
      'This is plain Bitcoin: a signet UTXO appears in the explorer. NITI calls it parent funding because this is where the first contract can resolve from.',
    ...
    oracleCopy:
      'That number is the outcome secret. Anyone who already holds the prepared package can add that secret to the adaptor and turn an almost-signature into a valid Bitcoin signature.',
    cetEyebrow: 'Scene 3',
    cetTitle: 'The parent closes through the correct-outcome CET.',
    cetCopy:
      'After attestation, the parent CET spends the funding. This step says: “the result arrived; this path won”.',
    ...
    bridgeCopy:
      'This is the cDLC trick: the secret that resolves the parent also completes the bridge. The bridge spends the CET output and creates the child funding. No magic, just a signature that only closes with the correct outcome.',
    bridgeCaption: 'Real bridge: one 7,000 sat input, one 6,500 sat child output, 500 sat fee.',
    ahaTitle: 'One revelation. Two effects.',
    ahaCopy:
      'Without the secret, the bridge is a cryptographic draft. With the secret, it becomes a normal Bitcoin transaction. That is why a cDLC can cascade contracts without the next step waiting for a signer to be online at event time.',
    failEyebrow: 'Fail closed',
    failTitle: 'If the wrong password appears, nothing opens.',
    failCopy:
      'The bundle does not only show the happy path. It records that a wrong outcome does not activate the edge, a missing package is not enough, and early refund respects the timelock.',
    holderEyebrow: 'Retainer',
    holderTitle: 'Alice, Bob, and watchtower reach the same txid.',
    holderCopy:
      'Each authorized holder carries the prepared package. When the oracle publishes the secret, any of them can complete the same bridge. They do not receive signer secrets; only the material needed to act after the outcome.',
    productEyebrow: 'Product demo',
    productTitle: 'Now replace “outcome” with BTC/USD price.',
    productCopy:
      'On testnet, the same mechanics can teach educational dollar exposure: the oracle reports price, the contract computes how many sats fit the target, and the payout never promises real money.',
    collateral: 'collateral',
    price: 'oracle price',
    target: 'educational target',
    claim: 'calculated claim',
    productBoundary:
      'Signet/testnet simulation. No peg, no redemption, no mainnet, no real-value custody.',
    proofEyebrow: 'Proof and replay',
    proofTitle: 'The screen is simple. The evidence is auditable.',
    proofCopy:
      'The site points to public txids, committed bundles, and the proof inventory. The visual layer explains; trust comes from replay.',
    testEyebrow: 'Try it yourself',
    testTitle: 'Do not sign in the browser. Run the test in the terminal.',
    testCopy:
      'The page only shows evidence and commands. Live execution remains explicit, local, and testnet/signet.',
    ...
    footer:
      'Educational signet/testnet demo. Nothing here is a financial offer, dollar guarantee, stablecoin, real custody, or mainnet flow.',
  },
};
...
function setLanguage(next: Language): void {
  ...
  for (const element of all<HTMLElement>('[data-i18n]')) {
  ...
  if (autoPlay) {
    select<HTMLButtonElement>('[data-play]').textContent = t('pause');
  }
}

function initProgress(): void {
  const bar = select<HTMLElement>('[data-progress]');
  const update = () => {
...
function initScenes(): void {
  ...
  for (const scene of all<HTMLElement>('[data-scene]')) {
...
function initAutoplay(): void {
  ...
  const scenes = all<HTMLElement>('[data-scene]');
  const stop = () => {
    autoPlay = false;
...
function initCommands(): void {
  ...
  for (const button of all<HTMLButtonElement>('[data-copy-command]')) {
...
function renderHolders(): void {
  const grid = select<HTMLElement>('[data-holder-grid]');
