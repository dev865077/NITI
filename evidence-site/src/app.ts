...
const copy: Record<Language, Record<string, string>> = {
  pt: {
    ...
    heroLede:
      'Um oracle revela um segredo. Esse mesmo segredo fecha o parent, ativa a bridge e cria o child. Tudo aparece como transação pública.',
    ...
    problemCopy:
      'Whitepapers e logs de prova importam, mas a maioria das pessoas precisa de um caminho visível: o dinheiro entra, o oracle fala, a próxima transação fica válida.',
    ideaKicker: 'A ideia',
    ideaTitle: 'Prepare o caminho antes do resultado existir.',
    ideaCopy:
      'O parent, a bridge e o child são preparados antes. A bridge existe, mas fica travada pelo outcome do oracle.',
    trickKicker: 'O truque',
    trickTitle: 'Um segredo completa dois passos.',
    trickCopy:
      'Antes da attestation, a assinatura da bridge não é uma witness Bitcoin válida. Depois que o oracle revela o scalar correto, a bridge completa.',
    ...
    chainCopy:
      'Estes são objetos de evidência signet/testnet commitados. Cada txid aponta para um explorer público e o bundle pode ser verificado localmente.',
    productKicker: 'Produto-demo',
    productTitle: 'Exposição educativa ao dólar, sem dinheiro real.',
    productCopy:
      'O cartão demo mapeia um alvo em dólar para sats usando o preço BTC/USD do oracle. É só educativo: sem peg, sem promessa de resgate, sem mainnet.',
    ...
    failCopy:
      'O bundle registra os casos negativos em vez de escondê-los: outcome errado rejeitado, pacote ausente rejeitado, refund antecipado bloqueado.',
    ...
    holderCopy:
      'Um prepared edge package permite que qualquer holder autorizado complete a mesma bridge após o oracle atestar. Nenhum holder recebe signer secrets.',
    proofKicker: 'Fronteira formal',
    proofTitle: 'A história é apoiada por provas, não só animação.',
    proofCopy:
      'Lean prova a camada protocolar da NITI. SPARK verifica a álgebra finita. O site separa claims de produto da evidência técnica.',
    ...
    testCopy:
      'O browser não assina nem faz broadcast. Ele mostra comandos e verifica evidência commitada. A execução signet ao vivo continua explícita.',
    ...
    boundary:
      'Fronteira: demo educativa em testnet. Sem depósito mainnet, sem stablecoin, sem garantia de resgate, sem recomendação de investimento.',
  },
  en: {
    play: 'Play',
    pause: 'Pause',
    ...
    heroLede:
      'An oracle reveals a secret. The same secret resolves the parent, activates the bridge, and creates the child. The path appears as public transactions.',
    ...
    problemCopy:
      'Whitepapers and proof logs matter, but most people need a visible path: money enters, the oracle speaks, the next transaction becomes valid.',
    ideaKicker: 'The idea',
    ideaTitle: 'Prepare the path before the result exists.',
    ideaCopy:
      'The parent, bridge, and child are prepared in advance. The bridge is present, but locked behind the oracle outcome.',
    trickKicker: 'The trick',
    trickTitle: 'One secret completes two steps.',
    trickCopy:
      'Before attestation, the bridge signature is not a valid Bitcoin witness. After the oracle reveals the matching scalar, the bridge completes.',
    ...
    chainCopy:
      'These are committed signet/testnet evidence objects. Every txid links to a public explorer and the bundle can be verified locally.',
    productKicker: 'Product demo',
    productTitle: 'Educational dollar exposure, without real money.',
    productCopy:
      'The demo card maps a dollar target to sats using the oracle BTC/USD price. It is educational only: no peg, no redemption promise, no mainnet funds.',
    testnetOnly: 'TESTNET ONLY',
    ...
    failCopy:
      'The evidence bundle records negative cases instead of hiding them: wrong outcome rejected, missing package rejected, early refund blocked.',
    ...
    holderCopy:
      'A prepared edge package lets any authorized holder complete the same bridge after the oracle attests. No holder receives signer secrets.',
    proofKicker: 'Formal boundary',
    proofTitle: 'The story is backed by proofs, not just animation.',
    proofCopy:
      'Lean proves the NITI protocol layer. SPARK checks the finite algebra. The site separates product claims from technical evidence.',
    ...
    testCopy:
      'The browser does not sign or broadcast. It shows commands and verifies committed evidence. Live signet execution remains explicit.',
    ...
    boundary:
      'Boundary: testnet educational demo only. No mainnet deposit flow, no stablecoin claim, no redemption guarantee, no investment advice.',
  },
};
...
function setLanguage(next: Language): void {
  ...
  for (const element of selectAll<HTMLElement>('[data-i18n]')) {
    const key = element.dataset.i18n;
    if (key && copy[next][key]) {
      element.textContent = copy[next][key] ?? key;
    }
  }
  select<HTMLButtonElement>('[data-lang-toggle]').textContent = next === 'pt' ? 'EN' : 'PT';
  if (autoPlay) {
    select<HTMLButtonElement>('[data-play]').textContent = copy[next].pause ?? 'Pause';
  }
}

function renderEvidence(): void {
  const rail = select<HTMLElement>('[data-tx-rail]');
  rail.replaceChildren(
    ...txEvidence.map((tx, index) => {
  ...
  const holderStrip = select<HTMLElement>('[data-holder-strip]');
  holderStrip.replaceChildren(
    ...holders.map((holder) => {
      const item = document.createElement('article');
  ...
  for (const key of Object.keys(commands) as Array<keyof typeof commands>) {
    const target = document.querySelector<HTMLElement>(`[data-command="${key}"]`);
    if (target) {
...
function initCopyButtons(): void {
  for (const button of selectAll<HTMLButtonElement>('[data-copy-command]')) {
    button.addEventListener('click', async () => {
      const key = button.dataset.copyCommand as keyof typeof commands;
...
function initTxidTool(): void {
  const input = select<HTMLInputElement>('[data-txid-input]');
  const link = select<HTMLAnchorElement>('[data-txid-link]');
  const update = () => {
...
function initProgress(): void {
  const progress = select<HTMLElement>('[data-progress]');
  const update = () => {
...
function initSceneObserver(): void {
  ...
  for (const scene of selectAll<HTMLElement>('[data-scene]')) {
    observer.observe(scene);
  }
}

function initAutoPlay(): void {
  const button = select<HTMLButtonElement>('[data-play]');
  const scenes = selectAll<HTMLElement>('[data-scene]');
  const stop = () => {
    autoPlay = false;
...
function initLanguageToggle(): void {
  select<HTMLButtonElement>('[data-lang-toggle]').addEventListener('click', () => {
...
function initCanvas(): void {
  const canvas = select<HTMLCanvasElement>('#hero-canvas');
...
function initProductMath(): void {
  ...
  select<HTMLElement>('[data-collateral]').textContent = `${collateral.toLocaleString()} sats`;
  select<HTMLElement>('[data-price]').textContent = '$60,000';
  select<HTMLElement>('[data-target]').textContent = '$3.90';
  select<HTMLElement>('[data-payout]').textContent = `${claim.toLocaleString()} sats`;
}
